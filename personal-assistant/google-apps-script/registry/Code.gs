...
function seedRegistryRows_(sheet, keyFields, rows) {
  ...
  const existing = {};
  getRegistryObjects_(sheet).forEach((row, index) => {
...
function getRegistryObjects_(sheet) {
...
function migrateLegacyRoutingToSettings_(ss, settingsSheet) {
  ...
  const existingSettings = getRegistryObjects_(settingsSheet);
  getRegistryObjects_(routing).forEach(route => {
