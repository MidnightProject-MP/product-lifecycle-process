...
| Skill | Purpose | Reads | Writes / Side Effects | Approval | Idempotency | Failure Behavior |
| --- | --- | --- | --- | --- | --- | --- |
| `record_graph_memory` | Record draft, send/log, discard, or flow-state graph observations. | Queue item or Flow_State input. | Hidden graph sheets. | No. | Drafts append lightweight events; approved send/log upserts verified W-node memory. | Best-effort callers do not block communication. |
