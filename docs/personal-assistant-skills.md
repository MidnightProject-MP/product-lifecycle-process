...
| `sensor_slack_pull_delta` | Pull recent messages from allowlisted Slack channels on a schedule. | Registry `MONITORED_SLACK_CHANNELS`, Automation `Config` cursors, Slack API. | `Unified_Event_Log`, Automation `Config` cursor rows. | No. | Channel/message timestamp becomes the source ID and dedupes through `append_unified_event`. | Logs per-channel errors and continues with other monitored channels. |
